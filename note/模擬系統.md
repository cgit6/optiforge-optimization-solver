...
- `mkp/configs/problems/`: 放轉檔後的標準題庫 yaml，由 `ProblemRepository` 讀取。
...
題庫讀取應該定義在這個模組，而不是放在 `Engine` 或 `Simulator` 中。  
`Engine` 負責組裝這個模組，`Simulator` 負責呼叫它。
...
亂數與可重現性的處理方式：
- 展開任務時，以 `numpy.SeedSequence(base_seed).spawn(n_tasks)[task_index]` 為每個 task 預先計算 `seed` 並存入 `RunTask`。
- 執行 task 時，直接以 `numpy.random.default_rng(RunTask.seed)` 建立 `Generator`，不在執行期重新推算。
- 這保證相同 base_seed 與相同 task 展開順序下結果可重現，且不同 task 之間亂數序列彼此獨立。
...
也就是說，`Engine` 的責任應該是：
- 建立 `ProblemRepository`
- 註冊可用 solver
- 建立 solver config loader
- 建立 Result writer / validator
- 產生可執行的 `Simulator / ExperimentRunner`

`Engine` 本身不直接負責題庫解析，也不直接執行演算法主迴圈。
...
此步驟負責解析使用者輸入，包含題庫來源、problem 清單、solver 清單、repeat 次數、base_seed、output 目錄等。所有實驗參數均由 CLI 定義，不依賴外部實驗設定檔。
...
3. `Engine` 根據 `ExperimentSpec` 組裝系統所需元件。
這裡會建立：
...
4. `Simulator / ExperimentRunner` 根據 `ExperimentSpec` 展開實驗任務。
系統會將：
...
展開成多個 `RunTask`，並在此步驟以 `numpy.SeedSequence(base_seed).spawn(n_tasks)[task_index]` 預先計算每個 task 的 `seed` 寫入 `RunTask`。  
每個 `RunTask` 代表一次獨立的 solver 執行單位，建立後即攜帶所有執行所需的識別資訊。

5. `Simulator / ExperimentRunner` 依序或平行執行各個 `RunTask`。
在執行每個 task 時，系統會先從 `ProblemRepository` 取得（或命中快取的）`ProblemModel`（傳入 `RunTask.dataset` 與 `RunTask.problem_id`）、載入對應 solver config yaml、以 `numpy.random.default_rng(RunTask.seed)` 建立該次 run 專屬的 `Generator`、實例化 solver，然後呼叫 solver 執行。
...
2. `Simulator` 向 `ProblemRepository` 取得 `ProblemModel`。
如果該 problem 已經載入過，可以直接從快取取得；否則由 `Repository` 從標準 yaml 讀取。
若 yaml 不存在、內容不合法、或 `best_known` 為 `null`，應立即回報錯誤並中止模擬。
...
- `ProblemModel` 快取（預先在平行啟動前全數載入，避免並行寫入快取的 race condition）
- `SolverRegistry`
- 題庫設定
- benchmark 設定
...
flowchart TD
    ...
    E --> F[從 ProblemRepository 取得 ProblemModel 並快取]
