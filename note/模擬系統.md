...
- `configs/problems/`: 放轉檔後的標準題庫 yaml；`Engine.build` 會掃描各 `<dataset>/*.yaml` 建立 **Catalog**（僅 metadata）與 **CatalogSummary**（索引摘要），並以 `ProblemRepository` **逐檔驗證**（可解析與 schema）；執行期 **`Simulator` 不再讀題目檔**，改由 **`ProblemBank`**（`multiprocessing.shared_memory` 唯讀陣列）依 `(dataset, problem_id)` 查表，且 **預設只為本次 `ExperimentSpec.problem_ids` 建立 SHM**（其餘題目僅驗證＋列入 catalog）。
...
也就是說，`Engine` 的責任應該是：
- 掃描 `problem_root` 下題庫目錄、建立 **Catalog／CatalogSummary**、以 `ProblemRepository` 對**每一題**做 fail-fast 驗證，並確認 `ExperimentSpec` 所引用題目皆在 catalog 內
...
3. `Engine`／模組 **`build`** 根據 `ExperimentSpec` 與設定根目錄組裝系統所需元件，並回傳 **`SimulationBundle`**（`spec`、`catalog_summary`、`catalog`、`problem_bank`、`solver_builders`、`default_rng`）；`Simulator` 由 bundle 的 **`new_simulator_with_seed`** 建立，或由 **`executeSimulator`** 編排。
這裡會建立：

- `ProblemRepository`（僅用於掃描後驗證與建 `ProblemBank`）
- `ProblemBank`（本次實驗題目的 SHM 唯讀資料）
- **Catalog／CatalogSummary**、**solver 註冊表**、**RNG 工廠**（列於 `SimulationBundle`）
