...
`題庫轉換命令指令`: 將目前 data 中的原始 txt / dat 題庫轉換成標準 yaml，保存到 `mkp/configs/problems/<dataset>/<problem_id>.yaml`。這是模擬執行前的前置步驟，不應在模擬主流程中動態進行。

`建立命令指令`: 所有實驗參數均由 CLI 直接指定，包含 dataset、problem、solver、repeat、seed、output 等。在執行模擬前，系統應先檢查：
- 所指定 problem 對應的 yaml 是否存在於 `mkp/configs/problems/`
- 所指定 solver 對應的參數 yaml 是否存在於 `mkp/configs/solvers/`
...
- `mkp/configs/problems/`: 放轉檔後的標準題庫 yaml；`Engine.build` 會掃描各 `<dataset>/*.yaml` 建立 **Catalog**（僅 metadata）與 **GameSetting**（索引），並以 `ProblemRepository` **逐檔驗證**（可解析與 schema）；執行期 **`Simulator` 不再讀題目檔**，改由 **`ProblemBank`**（`multiprocessing.shared_memory` 唯讀陣列）依 `(dataset, problem_id)` 查表，且 **預設只為本次 `ExperimentSpec.problem_ids` 建立 SHM**（其餘題目僅驗證＋列入 catalog）。
- `mkp/configs/solvers/`: 放演算法參數 yaml，每個 solver 一份，例如 `bsma_v1.yaml`、`gurobi.yaml`。
- `mkp/output/`: 放單次 run 與批次 summary 的輸出結果，子目錄名稱由執行時的 `experiment_id` 決定。

#### Problem YAML Schema

```yaml
# mkp/configs/problems/<dataset>/<problem_id>.yaml
...
```yaml
# mkp/configs/solvers/<solver_id>.yaml
...
- `Converter`: 將原始 txt / dat 題庫轉成統一 Problem YAML 格式，輸出至 `mkp/configs/problems/<dataset>/<problem_id>.yaml`。這是前置資料準備工具，不屬於模擬執行期間的主流程。由於不同題庫的原始格式不同（例如 WEISH、GK 的欄位排列方式各異），每個題庫應維護獨立的 parser 函數，由人工確認後加入。
- `Repository`: 根據 problem id，從 `mkp/configs/problems/` 中已存在的標準 yaml 載入並回傳 `ProblemModel`，並提供唯讀快取。
...
在執行模擬時，`Repository` 不應負責補做轉檔。若 `mkp/configs/problems/` 下指定題目的 yaml 不存在、格式錯誤、缺少必要欄位、或 `best_known` 為 `null`，系統應視為資料準備失敗並直接終止模擬。
...
solver 的核心輸入可以抽象成：
- `ProblemModel`
- 來自 `mkp/configs/solvers/<solver_id>.yaml` 的 solver 設定（含 `stop_condition` 與 `params`）
...
- 註冊可用 solver、建立 solver config loader、Result writer、validator
- 產生可執行的 `Simulator / ExperimentRunner`（注入 `ProblemBank`）

`Engine` 透過 `ProblemRepository` 完成題庫載入與驗證，但不負責執行期逐 task 讀檔；亦不直接執行演算法主迴圈。

**實作對齊（程式碼）**：`mkp/engine.py` 提供 `Engine.build(...)`：在已取得 `ExperimentSpec` 並通過 CLI 層 `preflight_validate`（指定題目／solver 檔案存在性）後，掃描題庫、驗證、建立 `ProblemBank`，再建立 `SolverRegistry`、`SolverConfigLoader`、`Validator`、`ResultWriter` 與注入 **`problem_bank`** 的 `Simulator`，回傳 **`SimulationBundle`**（`spec`、`simulator`、`game_setting`、`catalog_entries`、`problem_bank`）。`mkp/main.py` 於 `Engine.build` 後執行 `run_batch`，並在 **`finally` 呼叫 `bundle.problem_bank.close()`** 以 unlink shared memory。`mkp/app.py` 為轉匯至 `mkp.main` 的薄層，供測試與 `mkp.__init__` 延遲載入 `main`。
...
此步驟負責解析使用者輸入，包含題庫來源、problem 清單、solver 清單、repeat 次數、base_seed、output 目錄、可選的 `--execution-mode`（任務展開順序）等。所有實驗參數均由 CLI 定義，不依賴外部實驗設定檔。

2. `Command / App` 驗證輸入成功後，產生 `ExperimentSpec`。
`ExperimentSpec` 是本次實驗的統一描述物件，後續模組都依賴它，而不是直接依賴原始命令列參數。
這個驗證階段除了檢查參數格式，也應先確認：
- 所指定 problem 對應的標準 yaml 是否存在於 `mkp/configs/problems/`
- 所指定 solver 對應的參數 yaml 是否存在於 `mkp/configs/solvers/`

若其中任一缺少，則直接中止整批模擬。

3. `Engine` 根據 `ExperimentSpec` 與設定根目錄組裝系統所需元件，並回傳 **`SimulationBundle`**（內含同一個 `spec`、已組好的 `Simulator`、以及 `game_setting`／`catalog_entries`／`problem_bank`）。
這裡會建立：

- `ProblemRepository`（僅用於掃描後驗證與建 `ProblemBank`）
- `ProblemBank`（本次實驗題目的 SHM 唯讀資料）
- `SolverRegistry`（含 solver config loader 所需之對應關係）
- `SolverConfigLoader`
- `ResultWriter`
- `Validator`
- `Simulator / ExperimentRunner`

4. `Command / App`（或呼叫端）使用 bundle 中的 `Simulator`，根據同一個 `ExperimentSpec` 展開實驗任務。
系統會將：
...
4. `Simulator` 根據 solver id 從 `SolverRegistry` 建立 solver，並載入對應的 `mkp/configs/solvers/<solver_id>.yaml` 內容。
solver 的建立只依賴：
- `ProblemModel`
...
flowchart TD
    ...
    C -->|否| E[Repository 從 mkp/configs/problems 載入]
