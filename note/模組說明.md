...
限制 Engine 模組不支援多題庫、不支援多參數設定(僅能挑一組)

### Simulator 模組

可以支援多算法但是不支援多參數組合，支援多題目但不支援多題庫
- catalog_summary 屬性用來表示題庫 catalog 的索引摘要
- 這邊可以支持多算法也很奇怪，把這個功能拔掉 Simulator 就是跑 1 個題庫( n 個問題) 1 個算法 1 組參數設定
