import numpy as np
import pandas as pd
import copy as copy
import os
import inspect # 找匯入檔中物件名稱
import BSMA
import BSCA1
import BSCASMA2

class main:
    def __init__(self):
        # <===============读取.dat 文件===============>
        self.batch_directory = "data/SENT" # 問題資料夾
        self.batch_files_list = os.listdir(self.batch_directory) # 獲取檔案名稱
        self.batch_content = {} # 初始化問題

        self.run_time = 20 # 執行次數
        self.problem_set = ["01","02"] # 題目尾數
        self.algo_file = BSCASMA2 # 執行的檔案

        self.module_members = inspect.getmembers(self.algo_file, inspect.isclass) # 使用 inspect 模块获取模块中的自定义物件名称
        self.algorithm_list = np.array(self.module_members).T[0] # 列出所有算法["SMA1","SMA2"...]
        self.algorithm_select1 = [item for item in self.algorithm_list if item.startswith('BSMASCA_V1_RL_001_15')] # 獲取物件名稱  

        # 算法參數
        self.max_iter = 5000
        self.pop_size = 20

        self.output_folder = "output/sent"

        # 問題參數
        self.items = None # 物品數量
        self.dim = None # 物品數量
        self.glbal_best = None # 理論最佳解
        self.values = None # 利潤
        self.weights = None # 資源限制
        self.capacities = None # 容量限制 
    
    # 讀取格式
    def read_dat(self):
        for file_name in self.batch_files_list:
            file_path = os.path.join(self.batch_directory, file_name) # 建構完整的文件路徑
            
            if file_name.endswith(".dat"): # 設定條件來判斷是否要執行該檔
                with open(file_path, mode = "rt") as file:
                    content = file.readlines() # 
                    # print(content)
            self.batch_content[file_name] = content # 更新 batch_content

    # 轉換格式
    def transfrom(self,lines):
        # 將每行轉換為陣列
        data_arrays = np.array([],dtype=int)
        
        values = np.fromstring(lines, sep=' ')  # 移除换行符，并按空格分割
        row_array = np.array([value for value in values],dtype=int)
        # 移除最後一個未知元素
        data_arrays = np.append(data_arrays,row_array)
        return data_arrays        


    def exe(self):

        for iter in range(self.run_time):
            for prob in self.problem_set:
                file_path = os.path.normpath(os.path.join(self.batch_directory, f"sent{prob}.dat")) # 構建完整的文件路徑
                # 讀取題庫
                with open(file_path, 'r') as batch_file:
                    content = batch_file.read() # readlines() 跟 read() 有差 
                    # print(content)
                file_list = self.transfrom(content)
                # print(file_list)

                # # 問題參數
                self.items = int(file_list[0]) # 物品數量
                self.dim = int(file_list[1]) # 維度
                self.glbal_best = file_list[2] # 全局最佳
                self.values = file_list[3: 3 + self.items] # 利潤
                self.weights = file_list[3 + self.items: 3 + self.items +self.items * self.dim].reshape((self.dim,self.items)).T # 資源
                self.capacities = file_list[3 + self.items +self.items * self.dim: 3 + self.items +self.items * self.dim + self.dim] # 容量
                # print(self.items)


                # 調用每个物件
                for name in self.algorithm_select1:
                    if hasattr(self.algo_file, name):  # 檢查腳本中是否存在該物件
                        function_to_call = getattr(self.algo_file, name)
                        Gbest_sol, Gbest_fit = function_to_call(self.items,self.dim,self.glbal_best,self.values,self.weights,self.capacities).run() # 調用函數
                    else:
                        print(f"{name} 函數不存在於模組中")
                    
                    # <===========================輸出===============================>
                    if not os.path.exists(self.output_folder):
                        os.makedirs(self.output_folder)


                    output_folder = f"{self.output_folder}/{name}/SENT"
                    if not os.path.exists(output_folder):
                        os.makedirs(output_folder)

                    # 將 DataFrame 寫入 CSV 文件
                    result_name = f"{output_folder}/runtime_{iter}_prob_{prob}_algo_{name}.csv" # best 檔案名稱
                    
                    with open(result_name, 'w') as file:
                        file.write(str(Gbest_fit) + str(Gbest_sol))
                    print(f"result_runtime_{iter}_prob_{prob}_algo_{name},已保存")

if __name__ == "__main__":
    # 這裡放置只有在該文件作為主程序運行時才執行的代碼
    main().exe()