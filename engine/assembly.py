...
def build(
    *,
    ...
    problem_type = spec.problem_type # 問題類型(這個需要經過合法性測試)
    problem_repository = ProblemRepository(config_root=problem_root)
    print(f"[{problem_type}] scanning problem", file=sys.stderr, flush=True)
    raw_catalog, catalog_summary = scan_problem_catalog(problem_root)
    assert_spec_problems_in_catalog(spec, list(raw_catalog))
    print(f"[{problem_type}] loading problem", file=sys.stderr, flush=True)
    validate_spec_problems_in_repository(problem_repository, spec)
    print(f"[{problem_type}] loading solver config", file=sys.stderr, flush=True)
    solver_configs = SolverConfigsSnapshot.build(spec, Path(solver_root))
    print(f"[{problem_type}] building shared-memory problem bank", file=sys.stderr, flush=True)
    problem_bank = ProblemBank.build_for_spec(repository=problem_repository, spec=spec)
    print(f"[{problem_type}] engine build done.", file=sys.stderr, flush=True)
