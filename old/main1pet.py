...
class main:
    def __init__(self):
        ...
        self.algorithm_select = [item for item in self.algorithm_list if item.startswith('BSMASCA2_')] # 獲取物件名稱

        # 執行參數
        self.problem = ["2","3","4","5","6","7"]
        # self.problem = ["7"]
        self.output_folder = "output"  # 设置输出資料夾路径
        # 算法參數
        self.run_time = 30 # 執行參數
    ...
    def exe(self):
        ...
        for iter in range(self.run_time):
            for prob in self.problem:
                file_path = os.path.normpath(os.path.join(self.batch_directory, f"pet{prob}.dat")) # 構建完整的文件路徑
                # 讀取題庫
                ...
                for name in self.algorithm_select:
                    ...
                    BSAM_output_folder = f"{self.output_folder}/{name}/PET"
