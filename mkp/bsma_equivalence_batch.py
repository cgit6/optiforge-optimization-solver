...
def run_weish_equivalence_suite(
    *,
    ...
    if output_dir is None:
        output_dir = repo_root / "mkp/output/bsma_weish_suite"
...
def main(argv: list[str] | None = None) -> int:
    ...
    parser.add_argument(
        ...
        type=Path,
        default=None,
        help="專案根目錄（含 data/、mkp/；預設：此檔所在套件目錄的上一層）",
    )
    ...
    # __file__ = .../mkp/mkp/bsma_equivalence_batch.py → repo root = parents[1]（.../mkp）
    repo_root = args.repo_root if args.repo_root is not None else Path(__file__).resolve().parents[1]
