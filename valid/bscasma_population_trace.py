...
if __package__ in (None, ""):
    ...
    from ..solver.BSCASMA import BRLSMASCA2V100320050TestCore
...
def _load_old_bscasma_test_class(repo_root: Path) -> type[Any]:
    ...
    return getattr(module, "BRLSMASCA2_V1_003_20_050_test")
...
class NewTraceBSCASMATest(PopulationTraceMixin, BRLSMASCA2V100320050TestCore):
    def __init__(self, *args: Any, **kwargs: Any) -> None:
        BRLSMASCA2V100320050TestCore.__init__(self, *args, **kwargs)
        self.trace_rows: list[PopulationTraceRow] = []
...
def _safe_run_with_trace(solver: Any) -> TraceRunResult:
    ...
    BRLSMASCA2_V1_003_20_050_test 的 ``update_sma_weight`` 對 best=worst（S=0）情況沒做保護，會把 ``self.W`` 變成 NaN，
...
def build_parser() -> argparse.ArgumentParser:
    parser = argparse.ArgumentParser(description="Compare old/new BRLSMASCA2 v1.003 (test policy) population fitness traces.")
