...
def _load_old_bsca_class(repo_root: Path) -> type[Any]:
    old_dir = repo_root / "old"
    if not old_dir.is_dir():
        raise FileNotFoundError(f"old BSCA directory not found: {old_dir}")
    if str(old_dir) not in sys.path:
        sys.path.insert(0, str(old_dir))
    module = importlib.import_module("BSCA")
    return getattr(module, "BSCA")


def _make_old_trace_class(old_base: type[Any]) -> type[Any]:
    class OldTraceBSCA(PopulationTraceMixin, old_base):  # type: ignore[misc, valid-type]
        def __init__(
            self,
            items: int,
            dim: int,
            ...
            *,
            ...
            # 舊版 BSCA 沒有 seed 參數；雙源 RNG 必須在 __init__ 前 seed
            random.seed(seed)
            np.random.seed(seed)
...
class NewTraceBSCA(PopulationTraceMixin, BSCACore):
    def __init__(self, *args: Any, **kwargs: Any) -> None:
        BSCACore.__init__(self, *args, **kwargs)
        self.trace_rows: list[PopulationTraceRow] = []
...
def _run_old_trace(
    *,
    ...
    old_cls = _make_old_trace_class(_load_old_bsca_class(repo_root))
    random.seed(seed)
    np.random.seed(seed)
    solver = old_cls(
        problem.items,
...
def build_parser() -> argparse.ArgumentParser:
    parser = argparse.ArgumentParser(description="Compare old/new BSCA population fitness traces.")
    parser.add_argument("--repo-root", type=Path, default=Path(__file__).resolve().parents[1])
    parser.add_argument("--output-dir", type=Path, default=Path("output/bsca_population_trace"))
