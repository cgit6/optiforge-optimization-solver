...
class BSCACore:
    """與 old/BSCA.py::BSCA 相同邏輯（不 import old，供驗證與除錯對照）。"""
        ...
        for iter in range(self.max_iter):
            for i in range(self.pop_size):
                ...
                for j in range(self.items):
                    ...
                    # 維持 BSCA 原本 abs/括號位置：abs(r1*sin(r2)) * r3 * Gbest - sol
                    if r4 < 0.5:
...
@dataclass
class BSCASolver:
    """BSCA：內建與 old/BSCA::BSCA 相同演算法本體（BSCACore），不使用 import old。"""
    ...
    def solve(self, problem: ProblemModel, config: dict[str, Any], rng: np.random.Generator) -> SolveResult:
        ...
        if stop_condition.get("type") != "max_iterations":
            raise ValueError("bsca only supports stop_condition.type=max_iterations")
