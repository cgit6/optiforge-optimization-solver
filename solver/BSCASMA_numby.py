"""BRLSMASCA2 test 版的 Numba 加速版：與 ``BRLSMASCA2V100320050TestCore`` 主迴圈對齊。

為 **BSMA + BSCA2** 混合體，對齊時須同時遵守兩邊慣例：

- **BSMA 段**：``update_sma_weight`` 每列使用 ``np.random.random(items)`` 一次取向量（不可逐格 ``random()``）；
  ``sma_local`` 的 ``vb``／``vc`` 用 ``np.random.uniform(..., size=items)``；``pool`` 在每個物品維度內重填後 ``choice``（與 ``BSMA_numby`` 相同）。
- **BSCA2 段**：``sca_sin``／``sca_cos`` 用 ``math.pi``、``math.sin``／``math.cos``（與 ``BSCA2`` 參考版一致）。
...
``repair`` 第二段無 ``else: break``（與 ``BRLSMASCA2V100320050TestCore.repair`` 相同），沿用 ``BSCA2_numby._repair_bsca2_row_inplace``。
"""
...
def _update_sma_weight_inplace(W: np.ndarray, pop_fit: np.ndarray, pop_size: int, items: int) -> None:
    """與 ``BSMA_numby._bsma_main_loop_numba`` 內權重段及 ``BRLSMASCA2V100320050TestCore.update_sma_weight`` 相同：
...
def _sma_local_row(
    ...
    # 與 ``BSMA_numby`` / ``BRLSMASCA2V100320050TestCore.sma_local`` 相同：一次取向量 uniform
    vb[:] = np.random.uniform(-a, a, items)
    vc[:] = np.random.uniform(-b, b, items)
    # 與 ``BSMA_numby`` 相同：每個物品維度內重填 ``pool`` 再 ``choice``（不消耗額外 RNG，但與 njit 編譯路徑一致）
    for j in range(items):
        r = np.random.random()
        k = 0
...
class BRLSMASCA2V100320050TestNumbaCore:
    """與 ``BRLSMASCA2V100320050TestCore`` 相同前置；主迭代交給 Numba。"""
...
@dataclass
class BRLSMASCA2V100320050TestNumbaSolver:
    """BRLSMASCA2 Numba 變體；solver_id 預設 ``brlsmasca2_numba``。"""
    ...
    def solve(self, problem: ProblemModel, config: dict[str, Any], rng: np.random.Generator) -> SolveResult:
        ...
        if stop_condition.get("type") != "max_iterations":
            raise ValueError("brlsmasca2_numba only supports stop_condition.type=max_iterations")
        ...
        core = BRLSMASCA2V100320050TestNumbaCore(
            problem.items,
            problem.dim,
            problem.best_known,
            problem.values,
        ...
        return SolveResult(
            ...
            solver_id=str(config.get("solver_id", "brlsmasca2_numba")),
            seed=run_seed,
